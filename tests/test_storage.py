...
def test_repository_persists_portfolio_and_alert_data(tmp_path) -> None:
    ...
    repository.upsert_alert_rule(AlertRule("FPT", 90, 120))
    repository.upsert_alert_snapshots({"FPT": AlertSnapshot("NẮM GIỮ", "Tăng")})
    event = AlertEvent(
        "event-1",
    ...
    assert reopened.list_alert_rules() == [AlertRule("FPT", 90, 120)]
